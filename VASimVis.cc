...
Wt::WApplication::instance()->require("/sigma.js/src/renderers/canvas/sigma.canvas.extremities.def.js");  
 ...
 sim_rev->setDisabled(true);
 sim_rev->clicked().connect( std::bind( [=] () {
       if (simIndex >= 0) {
	 stepFromCache(-1);
	 sim_step->setDisabled(false);
	 doJavaScript("$('#input-display-container').animate({scrollLeft: " + std::to_string((simIndex % max_input_display_size) * 30) + " - $(window).width()/2}, 300)");
	 if (simIndex == cache_index - cache_length + 1)
	   sim_rev->setDisabled(true);
       }
     }));
 ...
 play_btn->clicked().connect( std::bind( [=] () {
	 ...
	 if (simIndex < input_string.length() - 1) {
	   stepFromCache(1);
	   doJavaScript("$('#input-display-container').animate({scrollLeft: " + std::to_string((simIndex % max_input_display_size) * 30) + " - $(window).width()/2}, 0)");
	   processEvents();
	 }
	 else {
	   playSim = false;
 ...
 //Wt::WApplication::instance()->require("//cdn.jsdelivr.net/jquery.scrollto/2.1.2/jquery.scrollTo.min.js");
...
void VASimViz::stepFromCache(int step_size) {
  ...
  // Cache new items if past 75% of 'cache_fetch_size'
  if (cache_index - simIndex <= .25 * cache_fetch_size && cache_index != input_string.length() - 1)
...
void VASimViz::addToJSCache(int numGraphs) {
  ...
  doJavaScript("setCachedGraphs(" + cache.dump() + ")");
  std::cout << "\tCache length is now " << cache_length << " graphs." << std::endl; 
  doJavaScript("$('#loading-graph-modal').modal('hide')");
  processEvents();
...
void VASimViz::loadDemoGraph(int index) {
  ...
  case 13:
    ...
    break;
  case 14:
    loadTextFile("VASim/donut.input");
    fn = "VASim/donut.anml";
    break;
  }
  doJavaScript("$('#loading-graph-modal').modal('hide');");
