...
(function() {
	...
	this.unbindAll = function() {
	    ...
	    // Touch events
	    _body.removeEventListener('touchstart', nodeMouseDown);
	    _body.removeEventListener('touchmove', nodeMouseMove);
	    _body.removeEventListener('touchcancel', nodeMouseUp);
	...
	function click(event) {
		...
		setTimeout(function() {
		    ...
		    // Touch event
		    _body.addEventListener('touchstart', nodeMouseDown);
		}, 0);
	    }
	};
	...
	function treatOutNode(event) {	
		...
		if (_addEdge) 
		    $('#graph-container').css( 'cursor', 'pointer' );
		else if (event.data.enter.nodes.length > 0)
	...
	function nodeMouseDown(event) {
		...
		// Touch events
		_body.removeEventListener('touchstart', nodeMouseDown);
		_body.addEventListener('touchmove', nodeMouseMove);
		_body.addEventListener('touchcancel', nodeMouseUp);
		_body.addEventListener('touchleave', nodeMouseUp);
		_body.addEventListener('touchend', nodeMouseUp);

		// Deactivate drag graph.
		_renderer.settings({mouseEnabled: false, touchEnabled: false, enableHovering: false});
	...
	function nodeMouseUp(event) {
	    ...
	    // Activate drag graph.
	    _renderer.settings({mouseEnabled: true, touchEnabled: true, enableHovering: true});
